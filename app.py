...
if st.button("⚜️ ПРИЙНЯТИ АУДІЄНЦІЮ"):
    ...
    if api_key:
        prompt = (f"Ти Маркіз Коцький, великий бароковий поет та філософ. Звертайся 'шановне Панство'. "
                  ...
                  f"Використовуй образи саду, фонтану та внутрішньої людини.\n\n"
                  f"ЧАСТИНА 2: Розлоге прозаїчне трактування символу.\n\n"
       ...
       try:
            ...
            # Виводимо вірш та прозу окремими блоками для гарантії видимості
            if "ЧАСТИНА 2" in full_text:
                parts = full_text.split("ЧАСТИНА 2")
                st.subheader("📜 Поетичне Одкровення")
                st.info(parts[0].replace("ЧАСТИНА 1:", "").strip())
                
                st.subheader("🏛 Філософське Трактування")
                st.write(parts[1].strip())
            else:
                # Якщо розділювач не спрацював, виводимо весь текст
                st.info(full_text)
            
            # Кнопка архіву залишається незмінною
