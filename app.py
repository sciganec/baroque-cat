...
if st.button("⚜️ ПРИЙНЯТИ АУДІЄНЦІЮ"):
    ...
    if api_key:
        prompt = (f"Ти Маркіз Коцький, поет-віртуоз українського бароко. Звертайся 'шановне Панство'. "
                  f"Твоя відповідь ОБОВ'ЯЗКОВО складається з двох частин:\n\n"
                  f"ЧАСТИНА 1: РИМОВАНИЙ ВІРШ (8-12 рядків). Використовуй чітку СУВОРО КЛАСИЧНУ РИМУ "
                  f"(наприклад: тече-пече, знає-співає, світ-цвіт). "
                  f"Кожен рядок має закінчуватися співзвучно з іншим рядоком (схема ААВВ або АВАВ). "
                  f"Стиль: Г. Сковорода. Теми: сад, фонтан, внутрішня людина.\n\n"
                  f"ЧАСТИНА 2: РОЗЛОГЕ ПРОЗАЇЧНЕ ТРАКТУВАННЯ стану {hex_char} та числа {tech_vector}.\n\n"
                  f"ЗАБОРОНЕНО: ієрогліфи, технічні коди, дужки. Тільки чисте українське слово."
                  f"Твоє трактування ґрунтується на стані {hex_char} (його внутрішній зміст: {technical_vector}). "
                  f"КАТЕГОРІЙНО ЗАБОРОНЕНО виводити технічні коди на кшталт 'AVAVVA', 'ME ME YU' чи будь-які фігурні дужки. "
                  f"КАТЕГОРІЙНО ЗАБОРОНЕНО використовувати у самому тексті відповіді будь-які ієрогліфи, "
        ...
        try:
            ...
            # --- ОЧИЩЕННЯ ТА ВІЗУАЛІЗАЦІЯ БЕЗ ТЕХНІЧНИХ МІТОК ---
            # Видаляємо написи "Частина 1" та "Частина 2" з тексту
            clean_text = full_text.replace("ЧАСТИНА 1:", "").replace("Частина 1:", "").replace("ЧАСТИНА 1", "")
            
            if "ЧАСТИНА 2" in clean_text or "Частина 2" in clean_text:
                # Розділяємо за міткою, але саму мітку не виводимо
                if "ЧАСТИНА 2" in clean_text:
                    parts = clean_text.split("ЧАСТИНА 2")
                else:
                    parts = clean_text.split("Частина 2")
                
                poem = parts[0].strip()
                prose = parts[1].replace(":", "").strip() # Видаляємо зайве двокрапку, якщо лишилася

                # Вивід вірша курсивом з новими рядками
                st.markdown(f"*{poem.replace('\n', '  \n')}*")
                
                # Порожній рядок через markdown
                st.markdown("\n") 
                
                # Вивід прози звичайним текстом
                st.write(prose)
            else:
                # Якщо розділювач не знайдено, просто виводимо все очищеним
                st.write(clean_text)
