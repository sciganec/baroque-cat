...
if st.button("⚜️ ПРИЙНЯТИ АУДІЄНЦІЮ"):
    ...
    if api_key:
       # СУВОРИЙ ПРОМПТ З ТЕХНІЧНИМ МАРКЕРОМ
        # Оновлений промпт для абсолютної чистоти мови

        prompt = (f"Ти Маркіз Коцький. Пиши ВИКЛЮЧНО українською мовою. "
                  f"Твоя відповідь має сувору структуру:\n\n"
                  f"1. РИМОВАНИЙ ВІРШ: 8 рядків, рима ААВВ. Кожен рядок має однаковий ритм. "
                  f"Стиль Сковороди. Жодних білих віршів!\n"
                  f"2. Розділювач: ###SPLIT###\n"
                  f"3. ТРАКТУВАННЯ: Глибока проза без латиниці.\n\n"
                  f"Основа: гексаграма {hex_char}. Тема: сродна праця. "
                  f"ЗАБОРОНЕНО: слово 'Частина', англійська мова, символи ієрогліфів.")

        prompt = (f"Ти Маркіз Коцький, бароковий поет. Звертайся 'шановне Панство'. "
                  f"Твоя відповідь ОБОВ'ЯЗКОВО має маркер ###SPLIT### між віршем і прозою.\n\n"
                  f"1. РИМОВАНИЙ ВІРШ: 8 рядків, рима ААВВ. Кожен рядок має однаковий ритм. "
                  f"Стиль Сковороди. Жодних білих віршів!\n"
                  f"###SPLIT###\n\n"
                  f"ЧАСТИНА 2: РОЗЛОГЕ ТРАКТУВАННЯ українською мовою про стан {hex_char}.\n\n"
                  f"Жодної латиниці! Тільки мова Сковороди."
                  f"Текст має бути КРИШТАЛЕВО ЧИСТИМ: лише високе бароко та філософія Григорія Сковороди. "
                  f"Трансформуй числові значення у метафори: сродна праця, фонтан, сад, дві натури, три світи. "
                  f"Розкажи про цей момент як про божественну симфонію, уникаючи технічного сміття."
                  f"КАТЕГОРІЙНА ЗАБОРОНА: Не використовуй жодного слова латиницею, англійською мовою чи технічних термінів у тексті. "
                  f"Назви гексаграм чи чисел також подавай лише українською. Тільки чисте, живе українське слово."
                  f"КАТЕГОРІЙНО ЗАБОРОНЕНО виводити технічні коди на кшталт 'AVAVVA', 'ME ME YU' чи будь-які фігурні дужки. "
                  f"КАТЕГОРІЙНО ЗАБОРОНЕНО використовувати у самому тексті відповіді будь-які ієрогліфи чи гексаграми. ")
        ...
        try:
            ...
            # Стабільна обробка виводу
            if "###SPLIT###" in full_text:
                parts = full_text.split("###SPLIT###")
                poem = parts[0].strip()
                prose = parts[1].strip()

                # Вивід вірша курсивом
                ...
                # Резерв на випадок збою маркера
                st.write(full_text)
